...
export class Moviedetails extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        `${process.env.REACT_APP_GET_MOVIE_FREE_API}/${id}?api_key=${process.env.REACT_APP_API_KEY}`
      )
      .then((res) => {
        this.setState({
          films: {
            ...res.data,
            genres: res.data.genres.map((e) => {
              return e.name;
            }),
          },
        });
  ...
  render() {
    ...
    console.log('hoho', this.state);
    const { films } = this.state;
    return (
      <div>
        <MyNavbar
          routeSignUp={() => this.routeChangeSignUp()}
          isLoggedIn={this.props.user.isLoggedIn}
          allFilm={this.props.allFilms}
          userImage={this.props.user.user.image}
        />
        <main className="cont">
          <div className={[style['section'], style['flex']].join(' ')}>
            ...
            >
              <img
                src={`${process.env.REACT_APP_BASE_URL_IMAGE}${films.poster_path}`}
                alt=""
              />
            </div>
            <div
            ...
            >
              ...
              >
                <div>
                  ...
                  <p className={style['desc']}>{films.release_date ?? ''}</p>
                </div>
                <div>
                  <p className={style['detail']}>Status</p>
                  <p className={style['desc']}>{films.status ?? ''}</p>
                </div>
                <div>
                ...
                <div>
                  <p className={style['detail']}>Genres</p>
                  <p className={style['desc']}>{films.genres?.join(', ')}</p>
                </div>
              </div>
          ...
          >
            ...
            <div className={[['line'], style['grid-3']].join(' ')}>
              {this.props.location.location.map((item) => (
                <div
                  className={[['card'], style['flex-col']].join(' ')}
                  key={item.id}
                >
                  <div
                    className={[
                  ...
                  >
                    <div
                      className={[style['card-img'], style['grow-1']].join(' ')}
                    >
                      <img src={`${urlImg}${item.image}`} alt="" />
                    </div>
                    <div
                      className={[style['card-desc'], style['grow-2']].join(
                        ' '
                      )}
                    >
                  ...
                  <div
                    className={[style['card-line-1'], style['grid-4']].join(
                      ' '
                    )}
                  >
                    <button
                  ...
                  >
                    ...
                    <Link to="" className={style['paddingr-1']}>
                      add to cart
                    </Link>
                  </div>
                </div>
            ...
            >
              <div className={[style['line-lit'], style['grow-3']].join(' ')}>
                <hr />
              </div>
              <div className={[style['line-lit'], style['grow-1']].join(' ')}>
